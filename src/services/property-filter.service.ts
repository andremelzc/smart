...
export class PropertyFilterService {
  ...
  static async searchProperties(
    ...
    try {
      ...
      const amenitiesType =
        await connection.getDbObjectClass("SYS.ODCINUMBERLIST");
      const amenitiesValue =
        amenitiesList.length > 0 ? new amenitiesType(amenitiesList) : null;

      const adults = typeof filters.adults === "number" ? filters.adults : null;
      const children =
        typeof filters.children === "number" ? filters.children : null;
      ...
      const outBinds = result.outBinds as {
        p_result_set?: oracledb.ResultSet<unknown>;
      };
