...
export const authStoredProcedures = {
  ...
  async registerWithCredentials(
    ...
    try {
      console.log("Ejecutando SP_REGISTER_WITH_CREDENTIALS con:", {
        email,
        firstName,
        lastName,
      });
  ...
  async validateCredentials(
    email: string,
    password: string
  ): Promise<AuthResult & { userData?: UserData }> {
    try {
      ...
      const userHashResult = (await executeQuery(getUserHashSql, {
        email,
      })) as { rows?: { PASSWORD_HASH: string }[] };

      console.log(
        "Resultado de búsqueda de hash:",
        JSON.stringify(userHashResult, null, 2)
      );

      if (!userHashResult.rows || userHashResult.rows.length === 0) {
        console.log(
          "No se encontró usuario con credenciales para email:",
          email
        );
        return {
          success: false,
          ...
          JSON.stringify(
            {
              outBinds: result?.outBinds
                ? {
                    ...
                    out_user_cursor: result.outBinds.out_user_cursor
                      ? "CURSOR_EXISTS"
                      : "NO_CURSOR",
                  }
                : "NO_OUTBINDS",
