...
const normalizeRecord = (input: Record<string, unknown> | undefined) => {
  ...
  if (!input) return map;
  Object.entries(input).forEach(([key, value]) => map.set(key.toLowerCase(), value));
  return map;
};
...
const computeItemsBounds = (markers: MarkerPoint[]): MapBounds | null => {
  ...
  if (!Number.isFinite(latMin) || !Number.isFinite(latMax) || !Number.isFinite(lngMin) || !Number.isFinite(lngMax)) {
    return null;
  }
...
export function PropertySearchMap({ items, bounds, loading, onBoundsChange }: PropertySearchMapProps) {
  ...
  const markers = useMemo<MarkerPoint[]>(() => {
    ...
    items.forEach((item, index) => {
      ...
      const identifier =
        getString(normalized, ['property_id', 'id', 'propertyid', 'prop_id']) ?? String(index);
  ...
  const initialCenter = useMemo(() => {
    if (bounds) {
      return [(bounds.latMin + bounds.latMax) / 2, (bounds.lngMin + bounds.lngMax) / 2] as [number, number];
    }
    if (markersBounds) {
      return [
  ...
  if (!isClient) {
    return <div className="flex h-full w-full items-center justify-center text-sm text-gray-dark-500">Cargando mapa…</div>;
  }
  ...
  return (
    <div className="relative h-full w-full overflow-hidden rounded-3xl border border-blue-light-150 bg-white shadow-sm">
      ...
      >
        ...
        {markers.map((marker) => (
          <Marker key={marker.id} position={marker.position} icon={createPriceIcon(marker.label)} />
        ))}
      </MapContainer>
