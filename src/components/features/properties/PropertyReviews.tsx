  ...
  if (!hasReviews) {
    return (
      <div>
        ...
        <p className="text-gray-600">Esta propiedad aun no tiene resenas.</p>
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-6">
        <Star className="w-6 h-6 fill-yellow-400 text-yellow-400" />
        <h3 className="text-xl font-semibold">
          {realAverageRating.toFixed(1)}  {realTotalCount} resena{realTotalCount !== 1 ? 's' : ''}
        </h3>
      </div>
      ...
      <div className="space-y-6">
        ...
        {actualReviews.length > 3 && (
          ...
          >
            {showAllReviews 
              ? 'Mostrar menos resenas' 
              : `Mostrar las ${actualReviews.length} resenas`
            }
          </button>
        )}
      </div>
