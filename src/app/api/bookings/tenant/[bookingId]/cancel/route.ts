...
export async function POST(
  ...
  const bookingId = Number.parseInt(bookingIdParam, 10);
    ...
    const { tenantNote } = (await request.json().catch(() => ({}))) as { tenantNote?: unknown };
    const normalizedTenantNote = typeof tenantNote === "string" ? tenantNote.trim() : null;
    ...
    const bookingRow = bookingQuery.rows?.[0] as ({ STATUS: string; TENANT_ID: number; CHECKIN_DATE: Date }) | undefined;
    ...
    if (bookingRow.TENANT_ID !== tenantId) {
      return NextResponse.json({ error: "No esta autorizado para cancelar esta reserva" }, { status: 403 });
    }

    if ((bookingRow.STATUS || "").toUpperCase() !== STATUS_ACCEPTED) {
      return NextResponse.json({ error: "Solo se pueden cancelar reservas aceptadas" }, { status: 409 });
    }
    ...
    if (checkinDate <= today) {
      return NextResponse.json({ error: "No se puede cancelar despues de la fecha de check-in" }, { status: 409 });
    }
