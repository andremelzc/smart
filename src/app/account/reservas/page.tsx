...
export default function ReservationsPage() {
  ...
  const createChatForReservation = useCallback((reservation: FormattedReservation) => {
    ...
    const baseMessage = `Hola ${reservation.hostName.split(" ")[0] || "anfitrion"}, me gustaria coordinar detalles sobre ${reservation.propertyName}.`;
    ...
    const updated = [
      conversation,
      ...existing.filter((item: { reservationId?: string }) => item.reservationId !== reservation.id),
    ];
  ...
  const reservations = useMemo<FormattedReservation[]>(() => {
    const formattedBookings: FormattedReservation[] = bookings.map((booking: TenantBooking) => {
      ...
      <section className="space-y-4">
        {reservations.length === 0 ? (
          ...
          reservations.map((reservation) => {
            ...
            const canStartChat = reservation.status === "current" || reservation.status === "upcoming";
